function [kernels, kVars, kStats, hitStats, compStats, trialOutcomesByCond, changeSidesByCond, changeIndicesByCond] = ...
  sessionKernelFromSaved(sessionData, trialIdx)
...
hitStats = struct;
for s = 1:2
  hitStats.nTrials(s) = numel(trialOutcomesAll{1, s});
  hitStats.nHits(s)   = hitStats.nTrials(s) - sum(trialOutcomesAll{1, s});

  % RF counts
  idxRF = (changeSidesAll{1, s} == 0);
  hitStats.nRFTrials(s) = sum(idxRF);
  hitStats.nRFHits(s)   = sum(trialOutcomesAll{1, s} == 0 & idxRF);

  % Left counts
  if lr.leftIsRF
    idxLeft = (changeSidesAll{1, s} == 0);   % RF is Left
  else
    idxLeft = (changeSidesAll{1, s} == 1);   % Opp is Left
  end
  hitStats.nLeftTrials(s) = sum(idxLeft);
  hitStats.nLeftHits(s)   = sum(trialOutcomesAll{1, s} == 0 & idxLeft);
end
