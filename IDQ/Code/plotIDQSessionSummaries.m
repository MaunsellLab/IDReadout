...
plotPsychometricPanel(axPsych, sessionSummary.psychometric.overall, 'Overall', sessionSummary.plotLimits);
...
for iDir = 1:3
    ...
    plotPsychometricPanel(ax, sessionSummary.psychometric.byDir(iDir), label, sessionSummary.plotLimits, iDir == 1);
end
...
if sb.present && sb.n > 0
    lines{end+1} = sprintf('  Side Bias: median %.2g, IQR [%.2g %.2g]', sb.median, sb.iqr(1), sb.iqr(2));
else
...
if db.present && db.n > 0
    lines{end+1} = sprintf('  Direction bias: median %.2g, IQR [%.2g %.2g]', db.median, db.iqr(1), db.iqr(2));
else
...
% -------------------------------------------------------------------------
function plotPsychometricPanel(ax, psych, panelTitle, plotLimits, displayLegend)

if nargin < 5
  displayLegend = true;
end
hold(ax, 'on');
plot(ax, [plotLimits.xLim(1) plotLimits.xLim(2)], [0.5 0.5], '-k', 'HandleVisibility', 'off');
if isfield(psych, 'fit') && ~isempty(psych.fit.xGrid) && any(isfinite(psych.fit.pFit))
  plot(ax, psych.fit.xGrid, psych.fit.pFit, '-b', 'LineWidth', 1.2, 'DisplayName', 'combined fit');
end
markerList = {'or', 'sb'};
for iGroup = 1:numel(psych.groups)
  ...
  errLow = y - lo;
  errHigh = hi - y;
  errorbar(ax, x, y, errLow, errHigh, markerList{iGroup}, 'LineStyle', 'none', 'LineWidth', 0.65, ...
    'MarkerSize', 6, 'MarkerFaceColor', 'auto', 'DisplayName', g.label);
  n = g.n(idx);
  for i = 1:numel(x)
    text(ax, x(i) - 1.5, y(i), sprintf('%d', n(i)), ...
      'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', 'FontSize', 9, 'Clipping', 'on');
  end
end
