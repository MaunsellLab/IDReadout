function sessionAnalysisAll = makeIDQSessionAnalysis(processedFile)
% makeIDQSessionAnalysis
%
...
fprintf('makeIDQSessionAnalysis: %s\n', sessionName);
...
tMS = makeTimeVectorMS(sessionHeader, nFrames);
...
% driftMinusNonNoiseByFrameTrial = noiseMeasures.driftMinusNonNoiseByFrameTrial;
dirNoiseByFrameTrial = noiseMeasures.dirNoiseByFrameTrial;

% Primary flat-readout predictor:
% changed-side dir1 + dir2 + dir3, averaged over the step window.
...
%% ------------------------------------------------------------------------
function tMS = makeTimeVectorMS(sessionHeader, nFrames)
...
% driftMinusNonNoiseByFrameTrial = nan(nFrames, nTrials);
...
for iTrial = 1:nTrials
  ...
  % Legacy diagnostic:
  % old drift-minus-nondrift convention, retained only for comparison.
  nonDriftDirs = setdiff(1:nDirs, driftDir);

  driftNoise = changedSideDirNoise(driftDir, :);
  nonDriftNoise1 = changedSideDirNoise(nonDriftDirs(1), :);
  nonDriftNoise2 = changedSideDirNoise(nonDriftDirs(2), :);

  % driftMinusNonNoiseByFrameTrial(:, iTrial) = ...
  %   (driftNoise - nonDriftNoise1 - nonDriftNoise2)';
...
% noiseMeasures.driftMinusNonNoiseByFrameTrial = driftMinusNonNoiseByFrameTrial;
