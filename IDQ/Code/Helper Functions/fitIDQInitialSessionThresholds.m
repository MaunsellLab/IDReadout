...
%
% Fit a fixed-beta, fixed-lapse Weibull threshold.
%
% Inputs:
...
for i = 1:nSessions
  ...
  fit = fitIDQSessionWeibullFixedBeta(trialTable.stepCoh(idx), trialTable.correct(idx), ...
  ...
  if numel(noisyCohThis) ~= 1
    error('fitIDQInitialSessionThresholds:NoisyCohMismatch', 'Expected one noisyStepCoh for session %d.', s);
  end
...
sessionFits = table(sessionIndex, sessionName, nTrials, nCorrect, meanCorrect, noisyStepCoh, ...
  threshold, alpha, negLogLikelihood, exitflag);
