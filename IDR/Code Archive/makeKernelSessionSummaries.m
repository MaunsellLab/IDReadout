...
for i = 1:numel(files)
  ...
  try
    compileKernelSessionSummary(kernelFile, 'summaryDir', summaryDir, 'replace', R.replace, ...
      'trackSideType', R.trackSideType, 'trackStepType', R.trackStepType);
  catch ME
    warning('makeKernelSessionSummaries:FailedFile', 'Failed on %s: %s', files(i).name, ME.message);
  end
end
end
