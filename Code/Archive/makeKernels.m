...
%       Kernel plot PDFs:     path/Plots/Kernels/<baseName>.pdf
  ...
  for k = 1:numel(dataFiles)
    ...
    for s = 1:2   % coherence step direction (inc/dec)
      ...
      [kernels(s, 1, :), kVars(s, 1), kStats(s, 1)] = meanPsychKernel(prefMat,  trialOutcomes{s}, prefCohNoisePC);
      [kernels(s, 2, :), kVars(s, 2), kStats(s, 2)] = meanPsychKernel(probeMat, trialOutcomes{s}, probeCohNoisePC);
    end
