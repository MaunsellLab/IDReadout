...
assert(~isempty(probeDirs), ...
  'plotSideTypeKernelAverage:NoProbeDirs', ...
...
for p = 1:nProbes
  ...
  assert(isfile(dataFile), ...
    'plotSideTypeKernelAverage:MissingAverageKernelPlotData', ...
    'Missing AverageKernelPlotData.mat for %s sideType "%s": %s', ...
    probeTag, sideType, dataFile);

  S = load(dataFile);

  assert(isfield(S, 'averageKernelPlotData'), ...
    'plotSideTypeKernelAverage:MissingStruct', ...
    'File does not contain averageKernelPlotData: %s', dataFile);

  D = S.averageKernelPlotData;

  assert(isfield(D, 'summarySideType'), ...
    'plotSideTypeKernelAverage:MissingSideType', ...
    'averageKernelPlotData.summarySideType missing in %s', dataFile);

  assert(strcmp(D.summarySideType, sideType), ...
    'plotSideTypeKernelAverage:WrongSideType', ...
    'Requested sideType "%s", but %s contains sideType "%s".', ...
    sideType, dataFile, D.summarySideType);

  assert(isfield(D, 'kernels') && ndims(D.kernels) == 3, ...
    'plotSideTypeKernelAverage:BadKernels', ...
  ...
  assert(isfield(D, 'stepTypeNames') && numel(D.stepTypeNames) == size(D.kernels, 1), ...
    'plotSideTypeKernelAverage:BadStepTypeNames', ...
    'stepTypeNames missing or inconsistent in %s', dataFile);

  assert(isfield(D, 'streamTypeNames') && numel(D.streamTypeNames) == size(D.kernels, 2), ...
    'plotSideTypeKernelAverage:BadStreamTypeNames', ...
    'streamTypeNames missing or inconsistent in %s', dataFile);

  assert(isfield(D, 'tMS') && numel(D.tMS) == size(D.kernels, 3), ...
    'plotSideTypeKernelAverage:BadTimeVector', ...
...
for p = 2:nProbes
  assert(isequal(plotData{p}.stepTypeNames, refStepTypeNames), ...
    'plotSideTypeKernelAverage:StepTypeMismatch', ...
    'stepTypeNames differ across probe directions.');

  assert(isequal(plotData{p}.streamTypeNames, refStreamTypeNames), ...
    'plotSideTypeKernelAverage:StreamTypeMismatch', ...
    'streamTypeNames differ across probe directions.');

  assert(numel(plotData{p}.tMS) == numel(refTMS) && ...
      all(abs(plotData{p}.tMS(:).' - refTMS) < 1e-9), ...
    'plotSideTypeKernelAverage:TimeMismatch', ...
    'tMS differs across probe directions.');
end
...
assert(nSteps == 2, ...
  'plotSideTypeKernelAverage:UnexpectedStepCount', ...
  'Expected 2 step types, found %d.', nSteps);

assert(nStreams == 2, ...
  'plotSideTypeKernelAverage:UnexpectedStreamCount', ...
  'Expected 2 stream types, found %d.', nStreams);
...
assert(overallYMax > overallYMin, ...
  'plotSideTypeKernelAverage:BadYLimits', ...
  'Could not determine valid y-limits.');

% ---- Timing for x-axis and gray patches ----
  D0 = plotData{1};
  
  assert(isfield(D0, 'msPerVFrame') && isfield(D0, 'firstPreStepMS') && ...
      isfield(D0, 'firstStepMS'), ...
...
for i = 1:numel(probeFolders)
  ...
  dataFile = fullfile(dataFolder, probeName, ...
    'AverageKernels', sideType, 'AverageKernelPlotData.mat');
...
if ~isempty(displayScaleCI)
  textStr = sprintf(['\\fontsize{6}%s: 0°: %.2f%%, %s: %.2f%%\n' ...
    ...
    valueLabel, prefIntegral, probeDirString(probeDirDeg), probeIntegral, ...
    ratioLabel, integralRatio, ...
    scaleLabel, scaleVal, scaleSEM, ...
    displayScaleCI.lo(sideTypeNum, stepType), ...
    displayScaleCI.hi(sideTypeNum, stepType));
else
  textStr = sprintf(['\\fontsize{6}%s: 0°: %.2f%%, %s: %.2f%%\n' ...
